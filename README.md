...
`discardsLateAnalysisResults` option: When `true` (the default), only one analysis result is in flight to the delegate at a time; results that arrive while the delegate is busy are dropped, not queued.
