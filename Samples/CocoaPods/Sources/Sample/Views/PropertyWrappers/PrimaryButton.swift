...
struct PrimaryButton<T: UIButton> {
    ...
    init(wrappedValue: T, title: String) {
        ...
        self.wrappedValue.setTitleColor(
            .white,
            for: .normal
        )
        self.wrappedValue.setTitle(
            title,
            for: .normal
        )
        self.wrappedValue.setBackgroundImage(
            .imageWithColor(color: .primaryButtonColor),
            for: .normal
        )
        self.wrappedValue.setBackgroundImage(
            .imageWithColor(color: UIColor.primaryButtonColor.withAlphaComponent(0.5)),
            for: .selected
        )
        self.wrappedValue.contentEdgeInsets = .init(
            top: 16,
            left: 16,
            bottom: 16,
            right: 16
